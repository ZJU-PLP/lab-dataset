...
from io import BytesIO  # 新增：处理二进制数据
...
# ===================== 新增：引入并初始化 petrel_client =====================
try:
    from petrel_client.client import Client
    # 初始化 petrel 客户端（配置文件路径替换为你的实际路径）
    PETREL_CONF_PATH = "~/petreloss.conf"
    client = Client(PETREL_CONF_PATH)
    # p-ceph 上 YCB 数据集的根路径（替换为你的实际路径）
    CEPH_YCB_ROOT = "s3://datafrontier/meijianbiao/YCB_Video_Dataset"
    print(f"✅ 成功初始化 petrel 客户端，远程数据集路径：{CEPH_YCB_ROOT}")
except Exception as e:
    print(f"❌ petrel 客户端初始化失败：{e}")
    exit(1)
...
class Dataset():
    ...
    def __init__(self, dataset_name, DEBUG=False):
        ...
        if dataset_name == 'train':
            self.add_noise = True
            # ===================== 修改：读取远程的 train_data_list.txt =====================
            train_list_url = f"{CEPH_YCB_ROOT}/datasets/ycb/dataset_config/train_data_list.txt"
            self.all_lst = self.read_remote_txt(train_list_url)
            ...
            # ===================== 修改：读取远程的 test_data_list.txt =====================
            test_list_url = f"{CEPH_YCB_ROOT}/datasets/ycb/dataset_config/test_data_list.txt"
            self.all_lst = self.read_remote_txt(test_list_url)
        print("{}_dataset_size: ".format(dataset_name), len(self.all_lst))
        # 不再使用本地 root，改用远程路径
        self.root = CEPH_YCB_ROOT
        self.sym_cls_ids = [13, 16, 19, 20, 21]

    # ===================== 新增：读取远程 txt 文件（如 train_data_list.txt） =====================
    def read_remote_txt(self, url):
        """读取 p-ceph 上的 txt 文件，返回行列表"""
        try:
            txt_data = client.get(url, no_cache=True)
            lines = txt_data.decode("utf-8").splitlines()
            lines = [line.strip() for line in lines if line.strip()]
            print(f"✅ 成功读取远程文件 {url}，共 {len(lines)} 行")
            return lines
        except Exception as e:
            print(f"❌ 读取远程文件失败 {url}：{e}")
            return []

    # ===================== 新增：读取远程图片文件（返回 PIL Image） =====================
    def read_remote_image(self, url):
        """读取 p-ceph 上的图片文件，返回 PIL Image 对象"""
        try:
            img_data = client.get(url, no_cache=True)
            img = Image.open(BytesIO(img_data))
            return img
        except Exception as e:
            print(f"❌ 读取远程图片失败 {url}：{e}")
            return None

    # ===================== 新增：读取远程 mat 文件（返回 mat 数据） =====================
    def read_remote_mat(self, url):
        """读取 p-ceph 上的 .mat 文件，返回 scipy 解析后的字典"""
        try:
            mat_data = client.get(url, no_cache=True)
            mat = scio.loadmat(BytesIO(mat_data))
            return mat
        except Exception as e:
            print(f"❌ 读取远程 mat 文件失败 {url}：{e}")
            return None
    ...
    def add_real_back(self, rgb, labels, dpt, dpt_msk):
        ...
        # ===================== 修改：读取远程深度图 =====================
        real_dpt_url = f"{self.root}/{real_item}-depth.png"
        di = self.read_remote_image(real_dpt_url)
        real_dpt = np.array(di) if di else np.zeros((480, 640))
        
        # ===================== 修改：读取远程 label 图 =====================
        bk_label_url = f"{self.root}/{real_item}-label.png"
        li = self.read_remote_image(bk_label_url)
        bk_label = np.array(li) if li else np.zeros((480, 640))
        
        bk_label = (bk_label <= 0).astype(rgb.dtype)
        bk_label_3c = np.repeat(bk_label[:, :, None], 3, 2)
        
        # ===================== 修改：读取远程背景图 =====================
        back_url = f"{self.root}/{real_item}-color.png"
        ri = self.read_remote_image(back_url)
        back = np.array(ri)[:, :, :3] * bk_label_3c if ri else np.zeros((480, 640, 3))
    ...
    def get_item(self, item_name):
        # ===================== 修改：读取远程深度图 =====================
        dpt_url = f"{self.root}/{item_name}-depth.png"
        di = self.read_remote_image(dpt_url)
        if di is None:
            return None
        dpt_um = np.array(di)
        
        # ===================== 修改：读取远程 label 图 =====================
        label_url = f"{self.root}/{item_name}-label.png"
        li = self.read_remote_image(label_url)
        if li is None:
            return None
        labels = np.array(li)
        ...
        # ===================== 修改：读取远程 meta.mat 文件 =====================
        meta_url = f"{self.root}/{item_name}-meta.mat"
        meta = self.read_remote_mat(meta_url)
        if meta is None:
            return None
        ...
        # ===================== 修改：读取远程彩色图 =====================
        rgb_url = f"{self.root}/{item_name}-color.png"
        ri = self.read_remote_image(rgb_url)
        if ri is None:
            return None
        if self.add_noise:
...
def main():
    global DEBUG
    DEBUG = True
    ds = Dataset('test', DEBUG=True)
    # 只取第 0 个样本测试
    datum = ds.__getitem__(0)
    if datum is not None:
        print("✅ 单样本读取成功！")
        print("样本包含字段：", list(datum.keys()))
    else:
        print("❌ 单样本读取失败！")
